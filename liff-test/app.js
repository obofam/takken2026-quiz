...
$('connect').onclick=async()=>{if(!ready)return;$('connect').disabled=true;try{if(!liff.isLoggedIn()){liff.login({redirectUri:location.origin+'/'});return;}await enter();}catch(e){$('message').textContent=window.MimioboMessages?.text(e.message)||'本人確認に失敗しました。';$('connect').disabled=false;}};
...
(async()=>{try{await new Promise(resolve=>{const script=document.createElement('script');script.src='/ui-messages.js';script.onload=resolve;script.onerror=resolve;document.head.append(script);});if(!window.TEST_LIFF_ID){$('connect').textContent='LINE接続の設定準備中';$('message').textContent='設定が終わると、このボタンから試せます。';return;}if(!window.liff)throw Error('LINEの読み込みに失敗しました。ページを開き直してください。');await liff.init({liffId:window.TEST_LIFF_ID});ready=true;$('connect').disabled=false;$('connect').textContent='LINEで連携して試す';if(location.pathname==='/ep10-preview.html'){location.replace(location.href);return;}if(liff.isLoggedIn())await enter();}catch(e){$('message').textContent=window.MimioboMessages?.text(e.message)||'本人確認に失敗しました。';$('connect').disabled=!ready;}})();
